...
export function SearchModal() {
  ...
  return (
    <AnimatePresence>
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-start justify-center pt-[10vh] bg-[#4a4a38]/30 backdrop-blur-sm p-4">
          <motion.div
          ...
          >
            <div className="flex items-center p-4 border-b border-[#e5e1d8] gap-3 bg-white">
              <Search className="text-[#a39f90]" size={18} />
              <input
                id="search-input"
                type="text"
                placeholder="Search items, tags, or categories..."
                className="flex-1 bg-transparent border-none outline-none text-base text-[#4a4a38] placeholder-[#a39f90]"
                value={query}
              ...
              {query.length === 0 ? (
                ...
                <div className="flex flex-col gap-2">
                  {results.map((res) => (
                    <div
                      key={res.item.id}
                      className="p-3 rounded-xl bg-[#fdfcf9] border border-[#e5e1d8] hover:border-[#8a9a5b] cursor-pointer flex justify-between items-center group transition-all shadow-sm"
                      onClick={() => navigateToItem(res.room?.id, res.furniture?.id)}
                    >
                      <div>
                        <div className="font-semibold text-[#4a4a38] text-sm">{res.item.name}</div>
                        <div className="text-[10px] text-[#a39f90] uppercase tracking-wide mt-1 flex items-center gap-2">
                          <span className="px-2 py-0.5 bg-[#f1eee6] rounded-full border border-[#d6d1c2] text-[#8a8678] font-semibold">{res.item.category}</span>
                          {res.item.tags.length > 0 && <span>· {res.item.tags.join(', ')}</span>}
                        </div>
                      </div>
                      <div className="text-xs font-medium text-[#a39f90] flex items-center gap-1 group-hover:text-[#8a9a5b] transition-colors">
                        <MapPin size={12} />
